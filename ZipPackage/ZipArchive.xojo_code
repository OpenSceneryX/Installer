...
Inherits ZipArchiveBase
	...
	#tag Method, Flags = &h0
		Function AddFakeDuplicate(entry as ZipEntry, newRawDOSPath as String, newDate as Date, newExtraField as ZipExtraField) As Integer
		  ...
		  fh.UInt16Value(28) = LenB(dosEncName)
		  if d <> nil then
		    // update item's date/time
		    fh.UInt16Value(12) = newDate.Second \ 2 + 32 * newDate.Minute + 2048 * newDate.Hour
		    fh.UInt16Value(14) = newDate.Day + 32 * newDate.Month + 512 *  Max(0, newDate.Year-1980)
		  end
		  ...
		  Exception exc as RuntimeException
		    me.errmsg = "An unexpected error occured while adding a duplicate item: "+ZipConfig.ShortDescription(exc)
		    return -1
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function AddItemByStreams(dataStream as SeqDataInputStream, dataLen as Integer, rsrcStream as SeqDataInputStream, rsrcLen as Integer, macBinaryHeader as MemoryBlock, rawDOSPath as String, extra as ZipExtraField, fileDate as Date, mayCompress as Boolean = true) As Integer
		  ...
		  idx = z_zip(dataStream, dataLen, rsrcStream, rsrcLen, mbHdr, rawDOSPath, fileDate, extra, mayCompress)
	...
	#tag Method, Flags = &h0
		Function AddItemWithRawPath(f as FolderItem, rawDOSPath as String, useMacBinary as Integer, mayCompress as Boolean = true) As Integer
		  ...
		  return z_addItem(new TTsFolderItem(f), rawDOSPath, useMacBinary, false, mayCompress)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Close() As Boolean
		  ...
		  if me.d.zipStream <> nil and not z_writeDirectory() then
		    error = true
		  end
		...
		Function Compact() As Boolean
		  ...
		  for i = 0 to Ubound(me.compactEntries)
		    ...
		    if lastOffset = readpos then
		      ...
		      if fh.UInt32Value(0) <> &h04034B50 then
		      ...
		      if Bitwise.BitAnd(flags,8) <> 0 then
		        ...
		        fh.UInt32Value(14) = hdr.UInt32Value(16)
		        fh.UInt32Value(18) = hdr.UInt32Value(20)
		        fh.UInt32Value(22) = hdr.UInt32Value(24)
		        fh.UInt16Value(6) = Bitwise.BitAnd(flags,65535-8)
		        forceRewrite = true
		      end
		      // make sure the values match those from the directory
		      if fh.UInt16Value(8) <> hdr.UInt16Value(10) or fh.UInt32Value(14) <> hdr.UInt32Value(16) or fh.UInt32Value(18) <> hdr.UInt32Value(20) or fh.UInt32Value(22) <> hdr.UInt32Value(24) or (flags <> hdr.UInt16Value(8)) and not me.d.isFakeEntry(hdr,flags) then
		      ...
		      n = 30 + fh.UInt32Value(18) + fh.UInt16Value(26) + fh.UInt16Value(28)
	...
	#tag Method, Flags = &h0
		Function MakeZipExtraField(macType as String, macCreator as String, macName as String) As ZipExtraField
		  ...
		  if n <> 0 then
		    data.UInt16Value(0) = &H2605
		  else
		    data.UInt16Value(0) = &H2705
		  end
		  data.UInt16Value(2) = data.Size-4
		  data.LittleEndian = false
		  data.UInt32Value(4) = &H5A504954 // "ZPIT"
		  if n <> 0 then
	...
	#tag Method, Flags = &h0
		Shared Function Version() As Double
		  return 3.040300 // v.mmffbb, with m=minor-version, f=bugfix-version, b=build(nonreleaseversion)
		End Function
	#tag EndMethod
	...
	#tag Note, Name = Acknowledgements
		...
		In 2016, Tom Buchler helped me a lot getting the 64 bit Windows
		version working.
		
		
		Enjoy!
		
		14 March 2003, updated 13 Jan 2016
	...
	#tag Note, Name = Plugins you will need
		...
		Alternatively, and by default, this code can use the zlib library which is pre-installed on
		Linux and OS X systems (inside /usr/lib/).
		
		For Microsoft Windows, however, you have to manually get the DLL and add it to your
		build app. See the Note "Read here for MS Windows" in the ZLibStream class for instructions.
